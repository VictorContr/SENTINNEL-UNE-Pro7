...
export const usePasantiasStore = defineStore('pasantias', () => {
  ...
  function getProgresoEstudiante(estudiante_id_sm_vc) {
    return materias_sm.value.map((materia) => {
      ...
      const conversacion = getConversacion(estudiante_id_sm_vc, materia.id_sm_vc)
      const reqAprobados = materia.requisitos.filter((r) =>
        conversacion.some(
          (m) =>
            m.requisito_id_sm_vc === r.id_sm_vc &&
      ...
      return {
        ...
        progreso_decimal: materia.requisitos.length > 0
        ...
        bloqueada: (() => {
          ...
          const anterior = materias_sm.value.find((m) => m.orden_sm_int === ordenActual - 1)
          const progAnterior = progreso_sm.value.find(
            (p) => p.estudiante_id_sm_vc === estudiante_id_sm_vc && p.materia_id_sm_vc === anterior?.id_sm_vc
          )
  ...
  function getDeployEstudiante(estudiante_id_sm_vc) {
    return deploys_sm.value.find((d) => d.estudiante_id_sm_vc === estudiante_id_sm_vc) ?? null
  }
  ...
  function enviarInforme({ materia_id_sm_vc, archivo_nombre_sm_vc, version_sm_vc, comentario_sm_vc, requisito_id_sm_vc }) {
    if (!auth.user) return
    const nuevo = {
    ...
    if (idx !== -1 && progreso_sm.value[idx].estado_aprobacion_sm_vc === 'PENDIENTE') {
  ...
  function responderCorreccion({
    ...
    if (estado_evaluacion_sm_vc === 'APROBADO') {
      ...
      if (idx !== -1) {
        ...
        progreso_sm.value[idx].fecha_aprobacion_sm_vc = new Date().toISOString()
      }
  ...
  function registrarDeploy({ url_produccion_sm_vc, archivo_codigo_nombre, documentacion_nombre }) {
    if (!auth.user) return
  ...
  function calcularPorcentaje(materia_enriquecida) {
    ...
    return Math.round((m.requisitos_aprobados_sm_int / m.total_requisitos_sm_int) * 100)
  }
