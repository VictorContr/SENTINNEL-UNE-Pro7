<template>
  <div class="traz-layout-root_sm_vc">
    ...
    <div v-if="cargando_sm_vc" class="flex flex-center flex-col" style="min-height: 50vh;">
      <q-spinner-dots color="teal-3" size="4em" />
      <span class="q-mt-md text-blue-grey-4 text-xs tracking-widest uppercase">Cargando expediente...</span>
    </div>
...
// ✅ FIX: Definimos la variable reactiva para el estado de carga
const cargando_sm_vc = ref(true)
...
watch(
  ...
  async (newId) => {
    ...
    if (newId) {
      cargando_sm_vc.value = true // Inicia el spinner
      try {
        await Promise.all([
        ...
        cargando_sm_vc.value = false // Oculta el spinner al terminar
      }
    } else {
...
<style scoped>
/* Tus estilos exactos se mantienen intactos */
.traz-layout-root_sm_vc { font-family: var(--sn-font-mono); max-width: 860px; }
.traz-ficha-est_sm_vc { display: flex; align-items: center; gap: 1rem; padding: 1rem 1.25rem; margin-bottom: 1rem; background: var(--sn-fondo-panel); border: 1px solid var(--sn-borde); border-radius: 12px; flex-wrap: wrap; }
.traz-ficha-est--muted_sm_vc { opacity: 0.92; }
.traz-ficha-avatar_sm_vc { width: 48px; height: 48px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 0.85rem; font-weight: 700; color: var(--sn-primario); background: var(--sn-surface-active); border: 1px solid var(--sn-borde-activo); flex-shrink: 0; }
.traz-ficha-body_sm_vc { flex: 1; min-width: 0; }
.traz-ficha-nombre_sm_vc { margin: 0 0 0.35rem; font-size: 1rem; font-weight: 700; letter-spacing: 0.04em; color: var(--sn-texto-principal); }
.traz-ficha-meta_sm_vc { display: flex; flex-wrap: wrap; gap: 0.65rem 1rem; }
.trz-meta-item_sm_vc { display: inline-flex; align-items: center; gap: 0.4rem; font-size: 15px; color: var(--sn-texto-terciario); font-family: var(--sn-font-sans); font-weight: 500; }
...
.trz-solo-nav_sm_vc { display: flex; flex-wrap: wrap; align-items: center; gap: 0.5rem; margin-bottom: 1rem; padding-bottom: 0.75rem; border-bottom: 1px solid var(--sn-borde); }
.trz-solo-conv-panel_sm_vc { border: 1px solid var(--sn-borde); border-radius: 14px; padding: 1rem 1.25rem 1.25rem; background: var(--sn-fondo-elevado); }
.trz-solo-empty_sm_vc { margin: 0; font-size: 0.75rem; color: var(--sn-texto-apagado); }
.trz-solo-materia-lbl_sm_vc { margin: 0 0 0.75rem; font-size: 0.72rem; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; color: var(--sn-acento-sec); }
.traz-conv-wrap--solo_sm_vc { margin-top: 0; }
.traz-stepper_sm_vc { background: transparent !important; border: 1px solid var(--sn-borde); border-radius: 14px; margin-bottom: 1.75rem; }
:deep(.q-stepper__tab) { font-family: var(--sn-font-mono); }
:deep(.q-stepper__title) { font-size: 0.82rem; font-weight: 600; color: var(--sn-texto-principal); letter-spacing: 0.04em; }
:deep(.q-stepper__caption) { font-size: 0.6rem; color: var(--sn-texto-terciario); font-family: var(--sn-font-sans); margin-top: 2px; }
:deep(.q-stepper__line) { border-color: var(--sn-borde) !important; }
:deep(.q-stepper__step-inner) { background: transparent; }
:deep(.q-step--disabled .q-stepper__title), :deep(.q-step--disabled .q-stepper__caption) { opacity: 0.4; }
.traz-step-item_sm_vc { border-bottom: 1px solid var(--sn-borde); }
.traz-step-item_sm_vc:last-child { border-bottom: none; }
.traz-conv-wrap_sm_vc { margin-top: 1rem; padding: 0 0 0.5rem; border: 1px solid var(--sn-borde); border-radius: 12px; background: var(--sn-fondo-panel); overflow: hidden; }
