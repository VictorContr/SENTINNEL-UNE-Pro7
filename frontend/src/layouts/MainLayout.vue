    ...
    <q-header class="sntnl-header_sm_vc" bordered>
      <q-toolbar class="sntnl-toolbar_sm_vc">
        ...
        <div class="sntnl-topbar-brand_sm_vc gt-sm">
          ...
          >
            ...
            <circle
              ...
              cy="18"
              r="4"
              :fill="isDark_sm_vc ? '#6fffe9' : '#0d7a6f'"
            />
          </svg>
        ...
        <div class="topbar-route-label_sm_vc">
          <q-icon
            :name="current_route_icon_sm_vc"
            size="16px"
            class="q-mr-xs route-icon_sm_vc"
          />
        ...
        <div class="role-indicator_sm_vc">
          <div
            class="role-dot_sm_vc"
            :style="{ background: rol_color_sm_vc }"
          />
        ...
        >
          ...
          >
            {{
              notif_store_sm_vc.conteoNoLeidas > 9
                ? "9+"
                : notif_store_sm_vc.conteoNoLeidas
            }}
          </q-badge>
          ...
          >
            <div class="dropdown-user-info_sm_vc">
              ...
              <div>
                ...
                <p class="dropdown-email_sm_vc">
                  {{ auth.user?.correo_sm_vc }}
                </p>
    ...
    >
      ...
      <q-scroll-area class="drawer-scroll_sm_vc">
        <q-list class="sntnl-nav-list_sm_vc">
          ...
          <q-item
            v-for="item in global_menu_sm_vc"
            :key="item.name"
            clickable
          ...
          >
            ...
            /></q-item-section>
            <q-item-section
              ><q-item-label class="nav-label_sm_vc">{{
                item.label
            ...
            <q-item-section
              v-if="
                ...
                notif_store_sm_vc.conteoNoLeidas > 0
              "
              side
            >
              <q-badge color="red-5" rounded>{{
                notif_store_sm_vc.conteoNoLeidas
              }}</q-badge>
            </q-item-section>
          </q-item>
          ...
          <template v-if="auth.isAdmin">
            ...
            <q-item
              v-for="item in admin_menu_sm_vc"
              :key="item.name"
              clickable
              :active="route_sm_vc.name === item.name"
              active-class="nav-item--active_sm_vc"
            ...
            >
              ...
              <q-item-section>
                ...
                <q-item-label caption class="nav-caption_sm_vc">{{
                  item.caption
                }}</q-item-label>
              </q-item-section>
            </q-item>
          ...
          <template v-if="auth.isProfesor">
            ...
            <q-item
              v-for="item in profesor_menu_sm_vc"
              :key="item.name"
              clickable
              :active="route_sm_vc.name === item.name"
              active-class="nav-item--active_sm_vc"
          ...
          <template v-if="auth.isEstudiante">
            ...
            <q-item
              v-for="item in estudiante_menu_sm_vc"
              :key="item.name"
              clickable
              :active="route_sm_vc.name === item.name"
              active-class="nav-item--active_sm_vc"
      ...
      <div class="drawer-footer_sm_vc">
        <div class="drawer-user-chip_sm_vc">
          ...
          <div class="drawer-user-info_sm_vc">
            ...
            <span
              class="drawer-user-rol_sm_vc"
              :style="{ color: rol_color_sm_vc }"
            >
              {{ auth.user?.rol_sm_vc }}
            </span>
          </div>
...
const route_meta_sm_vc = {
  ...
  "profesor-conversacion": {
    label: "Conversación de Documentos",
    icon: "forum",
  },
  "profesor-deploy": { label: "Deploy del Estudiante", icon: "rocket_launch" },
  "estudiante-trazabilidad": {
...
/* ═══ THEME TOGGLE BUTTON ═══ */
.theme-toggle-btn_sm_vc {
  color: var(--sn-texto-secundario);
}
.theme-toggle-btn_sm_vc:hover {
  color: var(--sn-primario);
}
...
.dropdown-sep_sm_vc {
  background: var(--sn-borde) !important;
}
:deep(.dropdown-list_sm_vc) {
  padding: 0.25rem 0;
}
:deep(.dropdown-item_sm_vc) {
  min-height: 38px;
  padding: 0.35rem 0.75rem;
}
:deep(.dropdown-item_sm_vc:hover) {
...
.status-dot_sm_vc {
  ...
  height: 5px;
  border-radius: 50%;
  background: var(--sn-exito);
  box-shadow: 0 0 6px var(--sn-exito);
  animation: pulse-dot_sm_vc 2s infinite;
}
@keyframes pulse-dot_sm_vc {
  0%,
  100% {
    opacity: 1;
  }
...
.nav-item_sm_vc :deep(.q-item__section--avatar) {
  min-width: 36px;
  color: var(--sn-texto-dim);
}
.nav-item_sm_vc:hover {
  background: var(--sn-surface-hover) !important;
}
.nav-item_sm_vc:hover :deep(.q-item__section--avatar) {
  color: var(--sn-acento-sec);
}
:deep(.nav-item--active_sm_vc) {
  background: var(--sn-surface-active) !important;
  border: 1px solid var(--sn-borde-activo);
}
:deep(.nav-item--active_sm_vc .q-item__section--avatar) {
  color: var(--sn-primario) !important;
}
:deep(.nav-item--active_sm_vc .nav-label_sm_vc) {
  color: var(--sn-primario) !important;
}
.nav-label_sm_vc {
  font-size: 0.78rem;
...
.drawer-user-info_sm_vc {
  flex: 1;
  display: flex;
  flex-direction: column;
  min-width: 0;
}
.drawer-user-name_sm_vc {
  font-size: 0.75rem;
  font-weight: 600;
  color: var(--sn-texto-principal);
  white-space: nowrap;
